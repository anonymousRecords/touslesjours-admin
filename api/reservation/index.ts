...
// 새 예약 추가하기
export const addReservation = async (reservation: Omit<Reservation, 'id'>): Promise<Reservation | null> => {
  const supabase = createClient();
  const { data, error } = await supabase
    .from('reservations')
    .insert(reservation)
...
// 예약 수정하기
export const updateReservation = async (id: string, reservation: 
  Partial<Omit<Reservation, 'id' | 'created_at' | 'updated_at'>>
