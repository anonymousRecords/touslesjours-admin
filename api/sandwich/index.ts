...
export const getSandwichSchedulesByPeriod = async (period: string): Promise<SandwichSchedule[]> => {
  const supabase = createClient();
  const { data, error } = await supabase.from('sandwich_schedule').select('*').eq('period', period);
...
export const getSandwichScheduleByDate = async (date: string): Promise<SandwichSchedule[]> => {
  const supabase = createClient();
  const { data, error } = await supabase.from('sandwich_schedule').select('*').eq('date', date);
...
export const deleteSandwichSchedule = async (date: string): Promise<void> => {
  const supabase = createClient();
  const { error } = await supabase.from('sandwich_schedule').delete().eq('date', date);
