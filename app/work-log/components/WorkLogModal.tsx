...
const WorkLogModal = ({ isOpen, onClose, userId, userShift, username }: WorkLogModalProps) => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [workLogs, setWorkLogs] = useState<WorkLog[]>([]);
  const [totalHours, setTotalHours] = useState(0);
  const [salary, setSalary] = useState(0);
  const [isAddingLog, setIsAddingLog] = useState(false);
  const [newLog, setNewLog] = useState<Omit<WorkLog, 'id' | 'created_at' | 'updated_at'>>({
    user_id: userId,
  ...
  const handleEditWorkLog = async (logId: number, updatedLog: Partial<WorkLog>) => {
    const editedLog = await updateWorkLog(logId, updatedLog);
    if (editedLog) {
      const updatedLogs = workLogs.map((log) => (log.id === logId ? editedLog : log));
      ...
      <div className="bg-white p-6 rounded-lg max-w-3xl w-full">
        <div className="flex flex-col justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={handlePreviousMonth} className="px-2 py-1 bg-gray-200 rounded">
              &lt;
            </button>
            <span className="text-xl font-bold">{format(currentDate, 'M월')}</span>
          ...
          <tbody>
            {workLogs.map((log, index) => (
              <tr key={log.id}>
                ...
                <td>{log.end_time}</td>
                <td>{log.hours}</td>
                <td>{workLogs.slice(0, index + 1).reduce((acc, cur) => acc + cur.hours, 0)}</td>
                <td>
                  <button
