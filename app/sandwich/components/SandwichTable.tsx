...
const SandwichTable: React.FC<SandwichTableProps> = ({ weekArray }) => {
  ...
  const handleDropDownChange = async (sandwichType: string, date: string, colorIndex: string) => {
    try {
  ...
  const handleFillAllData = async (date: string | null) => {
    ...
    try {
      const previousDate = format(new Date(date).valueOf() - 86400000, 'yyyy-MM-dd');
              ...
              {weekArray.map((date) => (
                <td key={date} className="text-center">
                  <Dropdown
                    ...
                    onSelect={(index: string) => handleDropDownChange(column, date, index)}
